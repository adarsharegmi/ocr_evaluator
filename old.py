...
class AWSTextractEngine:
    """AWS Textract OCR Engine with proper PDF support"""
    ...
    def extract_text(self, file_path: str) -> OCRResult:
        ...
        try:
            ...
            if PDFProcessor.is_pdf(file_path):
                # For PDF files, use asynchronous processing if file is large
                if file_size_mb > 5:  # Use async for files > 5MB
                    return self._process_pdf_async(file_content, start_time)
                else:
                    # Try synchronous processing first for smaller PDFs
                    try:
                        return self._process_pdf_sync(file_content, start_time)
                ...
                # For image files, use detect_document_text (synchronous)
                response = self.client.detect_document_text(
                    Document={'Bytes': file_content}
                )
                return self._extract_text_from_response(response, start_time, "detect_document_text")
    ...
    def _process_pdf_sync(self, file_content: bytes, start_time: float) -> OCRResult:
        """Try synchronous PDF processing with analyze_document"""
        try:
            response = self.client.analyze_document(
                Document={'Bytes': file_content},
                FeatureTypes=['TABLES', 'FORMS']
            )
            return self._extract_text_from_response(response, start_time, "analyze_document")
        except ClientError as e:
            # If sync fails, re-raise to trigger async processing
            raise e
    
    def _process_pdf_async(self, file_content: bytes, start_time: float) -> OCRResult:
        """Process PDF using asynchronous start_document_text_detection"""
        try:
            # For async processing, we need to upload to S3 first
            # Since we can't assume S3 access, we'll convert PDF to images locally
            # and process each image separately
            
            print("PDF too large or unsupported for direct processing, converting to images...")
            
            # Save temporarily to process with PDFProcessor
            import tempfile
            ...
            try:
                ...
                for i, image in enumerate(images):
                    ...
                    page_text, page_boxes = self._extract_text_and_boxes_from_response(response)
    ...
    def _extract_text_from_response(self, response: dict, start_time: float, method: str) -> OCRResult:
    ...
    def _extract_text_and_boxes_from_response(self, response: dict):
    ...
    def _process_pdf_with_s3_async(self, s3_bucket: str, s3_key: str) -> OCRResult:
        """
        Process PDF using true async processing with S3 (if you have S3 setup)
        This is the most efficient method for large PDFs
        """
        start_time = time.time()
        ...
        try:
            ...
            return OCRResult(
                engine_name="AWS Textract",
                text=f"Async processing error: {str(e)}",
                processing_time=time.time() - start_time
            )
...
class OCRComparator:
    ...
    def compare_engines(self, file_path: str, ground_truth: str = None) -> Dict:
...
def main():
    ...
    if comparator.engines:
        ...
        if os.path.exists(sample_file):
            ...
            # Run comparison
            results = comparator.compare_engines(sample_file, ground_truth)
